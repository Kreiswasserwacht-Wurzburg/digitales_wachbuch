...
async function removeGuard(guardService: GuardService) {
  try {
    await store.removeGuard({
      sentryId: props.sentryId,
      personId: guardService.guard.id,
      end: DateTime.now()
    })
  } catch (err) {
...
<template>
  <div class="card" v-for="guardService in props.guards">
    <div class="card-body">
      <h5 class="card-title">
        {{ guardService.guard.firstName }} {{ guardService.guard.lastName }}
      </h5>
      <h6 class="card-subtitle text-muted">{{ convertDateTimeToString(guardService.start) }}</h6>
    </div>
    <div class="card-footer text-end">
      <a class="btn btn-sm" href="#" @click.prevent="removeGuard(guardService)"
